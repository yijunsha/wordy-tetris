...
public class ShapeMovementController : MonoBehaviour {
    ...
    public void RotateClockWise(bool isCw)
    {
        ...
        TetrisShape TetrisShape = GetComponent<TetrisShape>();
        // Check if it's valid          
        if (Managers.Grid.IsValidGridPosition(this.transform)) // It's valid. Update grid.
        {
            Managers.Grid.UpdateGrid(this.transform);
            foreach (SpriteRenderer renderer in TetrisShape.GetComponentsInChildren<SpriteRenderer>()) {
                renderer.transform.Rotate(Vector3.back, rotationDegree);
            }
        }
